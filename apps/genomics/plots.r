...
plot_tcga_isoform_by_cohort <- function(df) {
  ggplot(
    df,
    aes(x = cohort_id, y = log2(as.numeric(normalized_counts) + 1), color = sample_type)
  ) +
    facet_grid(symbol + isoform_id ~ .) +
    geom_boxplot(fill = NA) +
    theme_bw() +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0)) +
    labs(
...
plot_gtex_isoform_by_tissue <- function(df) {
  ggplot(
    df,
    aes(x = tissue_id, y = log2(median_tpm + 1))
  ) +
